...
describe("worktree create", () => {
	...
	it("returns exists error when branch exists and not forced", async () => {
		...
		vi.spyOn(git, "getCommonRepoRoot").mockResolvedValue("/tmp/repo");
	...
	it("surfaces dependency install failures without failing worktree creation", async () => {
		vi.spyOn(git, "checkGitVersion").mockResolvedValue(true);
		vi.spyOn(git, "getCommonRepoRoot").mockResolvedValue("/tmp/repo");
		vi.spyOn(git, "getRepoName").mockReturnValue("repo");
		vi.spyOn(git, "listWorktrees").mockResolvedValue([]);
		...
		vi.spyOn(deps, "installDeps").mockResolvedValue({
			ok: false,
			error: "npm install failed: command not found",
			code: "DEP_INSTALL_FAILED",
		});
		vi.spyOn(configHelpers, "copyAllConfigFiles").mockResolvedValue([]);
		mockAccess.mockRejectedValue(Object.assign(new Error("no envrc"), { code: "ENOENT" }));
		mockMkdir.mockResolvedValue(undefined);

		const result = await worktreeCreate({ branch: "feat-deps" });
		expect(result).toEqual({
			ok: true,
			path: "/tmp/repo/.worktrees/repo-feat-deps",
			branch: "feat-deps",
			created: true,
			packageManager: "npm",
			depsInstallAttempted: true,
			depsInstallSuccess: false,
			depsInstallError: "npm install failed: command not found",
			configFilesCopied: [],
			gitignoreModified: true,
			direnvAllowRan: false,
			direnvAllowSuccess: true,
