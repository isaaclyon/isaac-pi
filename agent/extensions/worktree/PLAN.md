...
- **Steps (per worktree):**
  1. Resolve upstream base:
     - if `origin` exists, fetch `origin/<default-branch>`
     - otherwise, use local `<default-branch>`
  2. Rebase the worktree's branch onto that upstream base
...
Agent syncs with main:
  ...
  → Fetches origin/main, rebases feat/new-api onto origin/main, reinstalls deps
