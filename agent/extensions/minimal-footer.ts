/**
 * Vendored from @ogulcancelik/pi-minimal-footer@0.1.10.
 *
...
function parseGitStatus(
  output: string
): Pick<GitCache, "branch" | "dirty" | "ahead" | "behind"> {
...
function parseGitNumstat(output: string): GitLineDelta {
  let added = 0;
  let removed = 0;
  ...
  for (const line of output.split("\n")) {
    ...
    if (!addedText || !removedText) continue;
    if (addedText === "-" || removedText === "-") continue;

    added += parseInt(addedText, 10) || 0;
    removed += parseInt(removedText, 10) || 0;
  }

  return { added, removed };
}

function getGitDiffBase(): string {
  try {
    execSync("git rev-parse --verify HEAD >/dev/null 2>&1", {
      stdio: "ignore",
      timeout: 1000,
    });
    return "HEAD";
...
function getGitLineDelta(): GitLineDelta {
  ...
  try {
    const trackedDiff = execSync(`git diff --numstat ${base} -- 2>/dev/null`, {
      encoding: "utf8",
      timeout: 1000,
    });
    ({ added, removed } = parseGitNumstat(trackedDiff.trimEnd()));
  } catch {
    // Ignore and keep zero counts.
  }
      ...
      return {
        ...
        render(width: number): string[] {
          ...
          if (showBranch && gitCache?.branch) {
            ...
            if (gitCache.added) branchStr += theme.fg("success", ` +${gitCache.added}`);
            if (gitCache.removed) branchStr += theme.fg("error", ` -${gitCache.removed}`);
          }
