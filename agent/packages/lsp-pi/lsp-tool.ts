...
export function normalizeExecuteArgs(thirdArg: unknown, fourthArg: unknown, fifthArg: unknown): ExecuteArgs {
  // Runtime >= 0.51: (signal, onUpdate, ctx)
  if (isContextLike(fifthArg)) {
    return {
      signal: isAbortSignalLike(thirdArg) ? thirdArg : undefined,
      onUpdate: typeof fourthArg === "function" ? fourthArg as ExecuteArgs["onUpdate"] : undefined,
      ctx: fifthArg,
    };
  }

  // Runtime <= 0.50: (onUpdate, ctx, signal)
  if (isContextLike(fourthArg)) {
    return {
      signal: isAbortSignalLike(fifthArg) ? fifthArg : undefined,
      onUpdate: typeof thirdArg === "function" ? thirdArg as ExecuteArgs["onUpdate"] : undefined,
      ctx: fourthArg,
    };
  }
...
Use bash to find files: find src -name "*.ts" -type f`,
    ...
    async execute(_toolCallId, params, signalArg, onUpdateArg, ctxArg) {
      const { signal, onUpdate, ctx } = normalizeExecuteArgs(signalArg, onUpdateArg, ctxArg);
