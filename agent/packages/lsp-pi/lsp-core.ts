...
async function spawnChecked(cmd: string, args: string[], cwd: string): Promise<SpawnCheckResult> {
  try {
    const child = spawn(cmd, args, { cwd, stdio: ["pipe", "pipe", "pipe"] });
    const stderr: string[] = [];
    child.stderr.on("data", (chunk: Buffer) => stderr.push(chunk.toString("utf8").trim()));

    // If the process exits immediately (e.g. unsupported flag), preserve why it failed.
    return await new Promise((resolve) => {
      ...
      const finish = (value: SpawnCheckResult) => {
        if (settled) return;
        settled = true;
      ...
      const onExit = (code: number | null, signal: NodeJS.Signals | null) => finish({
        error: stderr.filter(Boolean).join("\n") || `${cmd} exited immediately (${code ?? signal ?? "unknown"})`,
      });
      const onError = (error: Error) => finish({ error: error.message });

      child.once("exit", onExit);
      child.once("error", onError);
      timer = setTimeout(() => finish({ process: child }), 200);
      timer.unref?.();
    });
  } catch (error) {
...
async function spawnWithFallback(cmd: string, argsVariants: string[][], cwd: string): Promise<ChildProcessWithoutNullStreams | undefined> {
  ...
  for (const args of argsVariants) {
    const result = await spawnChecked(cmd, args, cwd);
    if (result.process) return result.process;
    if (result.error) errors.push(`${cmd} ${args.join(" ")}: ${result.error}`);
  }
...
export const LSP_SERVERS: LSPServerConfig[] = [
  ...
  },
  {
    ...
    findRoot: (f, cwd) => {
      ...
      const configuredRoot = findRoot(f, cwd, ["package.json", "tsconfig.json", "jsconfig.json"]);
      if (configuredRoot) return configuredRoot;
      const relative = path.relative(path.resolve(cwd), path.resolve(f));
      return relative !== "" && !relative.startsWith(`..${path.sep}`) && relative !== ".." ? path.resolve(cwd) : undefined;
    },
    spawn: async (root) => {
...
export class LSPManager {
  ...
  private async initClient(config: LSPServerConfig, root: string): Promise<LSPClient | undefined> {
    ...
    let handle: Awaited<ReturnType<LSPServerConfig["spawn"]>>;
    let capturedStderr: string[] = [];
    try {
      handle = await config.spawn(root);
      if (!handle) {
        this.broken.add(k);
        this.initErrors.set(k, `${config.id} executable was not found`);
        return undefined;
      }

      const initOptions = handle.initOptions;
      ...
      conn.onRequest("workspace/configuration", () => [initOptions ?? {}]);
      ...
      if (initOptions) {
        conn.sendNotification("workspace/didChangeConfiguration", { settings: initOptions });
      }
      this.initErrors.delete(k);
      return client;
    } catch (error) {
      this.broken.add(k);
      const message = error instanceof Error ? error.message : String(error);
      const detail = [message, ...capturedStderr].filter(Boolean).join("\n");
      this.initErrors.set(k, detail || `${config.id} failed to initialize`);
  ...
  private explainNoLsp(absPath: string): string {
    ...
    for (const config of LSP_SERVERS) {
      ...
      const root = config.findRoot(absPath, this.cwd);
      if (!root) continue;
      const error = this.initErrors.get(this.key(config.id, root));
      if (error) return `${config.id} failed to initialize for ${root}: ${error}`;
    }
