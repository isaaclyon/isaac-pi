...
import { normalizeExecuteArgs } from "../lsp-tool.js";
...
test("tool execution args: supports current runtime order", () => {
  const signal = new AbortController().signal;
  const onUpdate = () => {};
  const ctx = { cwd: "/tmp/project" };
  const normalized = normalizeExecuteArgs(signal, onUpdate, ctx);
  assertEquals(normalized.signal, signal);
  assertEquals(normalized.onUpdate, onUpdate);
  assertEquals(normalized.ctx, ctx);
});

test("tool execution args: supports legacy runtime order", () => {
  const signal = new AbortController().signal;
  const onUpdate = () => {};
  const ctx = { cwd: "/tmp/project" };
  const normalized = normalizeExecuteArgs(onUpdate, ctx, signal);
  assertEquals(normalized.signal, signal);
  assertEquals(normalized.onUpdate, onUpdate);
  assertEquals(normalized.ctx, ctx);
});
...
test("typescript: falls back to cwd when no config files exist", async () => {
  await withTempDir({
    "script.ts": "const x = 1;",
  }, async (dir) => {
    const server = LSP_SERVERS.find(s => s.id === "typescript")!;
    const root = server.findRoot(join(dir, "script.ts"), dir);
    assertEquals(root, dir, "Should use cwd for standalone TypeScript files");
  });
});
...
test("stop boundary: does not search above cwd", async () => {
  await withTempDir({
    ...
    assertEquals(root, projectDir, "Should fall back to cwd without finding package.json above it");
  });
});
...
test("edge: no marker files uses cwd", async () => {
  await withTempDir({
    "random.ts": "const x = 1;",
  }, async (dir) => {
    const server = LSP_SERVERS.find(s => s.id === "typescript")!;
    const root = server.findRoot(join(dir, "random.ts"), dir);
    assertEquals(root, dir, "Should use cwd when no marker files exist");
  });
});
