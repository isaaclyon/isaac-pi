...
This local vendored copy writes best-effort JSONL lifecycle logs for subagent
spawns, pane creation, command injection, status transitions, exit polling,
summary extraction, and steer delivery.

Default log path:

```bash
~/.pi/agent/logs/subagents/YYYY-MM-DD.jsonl
```

Useful tail command:

```bash
tail -f ~/.pi/agent/logs/subagents/$(date +%F).jsonl | jq .
```

Environment overrides:

- `PI_SUBAGENT_LOG_DIR=/path/to/logs` — choose a different log directory.
- `PI_SUBAGENT_LOG_VERBOSE=1` — include full command/task strings instead of
...
| `tools`       | string  | Comma-separated native or extension tools. Common Codex tools include `exec_command`, `write_stdin`, `apply_patch`, `view_image`, `web_run`, and `imagegen`; Pi tools include `read`, `bash`, `edit`, `write`, `grep`, `find`, and `ls`                                                                                                                                                                             |
