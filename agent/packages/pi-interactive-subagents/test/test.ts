...
describe("subagent discovery", () => {
		...
		await withIsolatedAgentEnv(async ({ projectAgentsDir }) => {
			writeAgentFile(
				...
				[
					...
					"model: llama-swap/test-interactive-true",
					"interactive: true",
				].join("\n"),
			);
			writeAgentFile(
				...
				[
					...
					"model: llama-swap/test-interactive-false",
					"interactive: false",
				].join("\n"),
			);
		...
		await withIsolatedAgentEnv(async ({ projectAgentsDir }) => {
			writeAgentFile(
				...
				[
					...
					"model: llama-swap/test-interactive-unset",
				].join("\n"),
			);
	...
	it("hides disable-model-invocation agents from listings but keeps direct loading", async () => {
		await withIsolatedAgentEnv(async ({ projectAgentsDir }) => {
			...
			assert.equal(loaded.model, "llama-swap/test-hidden");
	...
	it("lets a hidden project agent shadow a visible global agent", async () => {
		await withIsolatedAgentEnv(
			async ({ projectAgentsDir, globalAgentsDir }) => {
				...
				assert.equal(loaded.model, "llama-swap/test-project");
...
describe("subagent-done.ts", () => {
	...
	describe("subagent_done tool", () => {
		it("refuses to exit without a non-empty summary", async () => {
			...
			process.env.PI_SUBAGENT_SESSION = join(dir, "child.jsonl");
			try {
				const { api, registeredTools } = createMockExtensionApi();
				subagentDoneExtension(api);
				const tool = registeredTools.find((tool) => tool.name === "subagent_done");
				assert.ok(tool, "expected subagent_done to be registered");
				...
				assert.equal(result.details.error, "summary_required");
				assert.throws(() => readFileSync(`${process.env.PI_SUBAGENT_SESSION}.exit`, "utf8"));
			} finally {
		...
		it("writes a done sidecar with the provided summary", async () => {
			const dir = createTestDir();
			const previousSession = process.env.PI_SUBAGENT_SESSION;
			process.env.PI_SUBAGENT_SESSION = join(dir, "child.jsonl");
			try {
				...
				assert.deepEqual(
					...
					{ type: "done", summary: "Launch succeeded." },
				);
			} finally {
				restoreEnvVar("PI_SUBAGENT_SESSION", previousSession);
				rmSync(dir, { recursive: true, force: true });
...
describe("cmux.ts interpretExitSidecar", () => {
	...
	it("decodes done payload summaries", () => {
		assert.deepEqual(
			interpretExitSidecar({ type: "done", summary: "  Done summary.  " }),
			{
				reason: "done",
				exitCode: 0,
...
describe("subagent interruption", () => {
	...
	it("renders a clear provider/agent error when errorMessage is set", () => {
		...
		assert.match(presentation, /Error: provider\/agent error/);
...
describe("cmux.ts", () => {
	...
	describe("model preference config", () => {
		describe("resolveModelPreference", () => {
			...
			it("returns tier model when agentTiers + modelTiers match", () => {
				// Tier-based override: agentName -> agentTiers[agentName] -> tier -> modelTiers[tier]
				// With config: returns tier model. Without config: falls through to frontmatter.
				const result = subagentsModule.__test__.resolveModelPreference(
					undefined,
					"frontmatter-model",
					"coder",
				);
				assert.ok(result === "frontmatter-model" || typeof result === "string");
			});

			it("returns defaultModel when no agent and no frontmatter model", () => {
				// With config: returns config.defaultModel. Without config: returns undefined.
				const result = subagentsModule.__test__.resolveModelPreference(
					undefined,
					undefined,
					undefined,
			...
			it("falls through to frontmatter when tier name not in modelTiers", () => {
				// If agentTiers maps to a tier not in modelTiers, should fall through.
				// Without config: returns frontmatter. With config: falls through if tier missing.
				const result = subagentsModule.__test__.resolveModelPreference(
					undefined,
					"frontmatter-model",
					"unknown-agent",
		...
		describe("getModelPreferenceConfig", () => {
			it("returns null when settings.json doesn't exist", () => {
				...
				// Should be null (no settings) or a valid config object with expected shape
				if (result === null) return;
				assert.ok(typeof result === "object", "should return object or null");
				assert.ok(
					"defaultModel" in result ||
						"modelTiers" in result ||
