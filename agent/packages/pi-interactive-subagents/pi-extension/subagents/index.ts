...
function getSubagentLogDir(): string {
	return (
		process.env.PI_SUBAGENT_LOG_DIR ??
		join(getAgentConfigDir(), "logs", "subagents")
	);
}

function getSubagentLogFile(): string {
	return join(
		getSubagentLogDir(),
...
function previewText(value: string): {
	...
	const preview =
		singleLine.length <= 240
			? singleLine
			: `${singleLine.slice(0, 120)} … ${singleLine.slice(-80)}`;
	return {
		length: value.length,
		preview,
		...(process.env.PI_SUBAGENT_LOG_VERBOSE === "1" ? { full: value } : {}),
	};
}

function bestEffortScreenTail(surface: string, lines = 80): string | undefined {
	try {
...
function logSubagent(
	...
	try {
		const logFile = getSubagentLogFile();
...
function getModelPreferenceConfig(): ModelPreferenceConfig | null {
	...
	try {
		...
		return {
			defaultModel:
				typeof subagent.defaultModel === "string"
					? subagent.defaultModel
					: undefined,
			modelTiers:
				typeof subagent.modelTiers === "object" && subagent.modelTiers != null
					? (subagent.modelTiers as Record<string, string>)
					: undefined,
			agentTiers:
				typeof subagent.agentTiers === "object" && subagent.agentTiers != null
...
/**
 * Load the model registry from ~/.pi/agent/models.json.
 * Extracts all model IDs into a cached Set for O(1) validation lookups.
 * Returns null if the file is missing, malformed, or has zero models.
 */
function loadModelRegistry(): Set<string> | null {
	...
	try {
		...
		const providers =
			(data.providers as Record<string, { models: { id?: string }[] }>) ?? {};
		...
		// Malformed JSON — treat as missing registry
		return null;
	}
}
 ...
 * Handles provider prefix stripping (e.g. "llama-swap/researcher" → "researcher").
 * Returns { valid: true } or { valid: false, hint: string }.
 */
function validateModel(
	modelAlias: string,
	registry: Set<string> | null,
): { valid: true } | { valid: false; hint: string } {
	if (!registry) return { valid: true }; // Can't validate, allow through

	// Strip provider prefix if present (e.g. "llama-swap/researcher" → "researcher")
	const bareModel = modelAlias.includes("/")
		? modelAlias.split("/").pop()!
		: modelAlias;

	if (registry.has(bareModel)) return { valid: true };

	const list = [...registry].sort().slice(0, 20).join(", ");
	const truncated = registry.size > 20 ? " (truncated)" : "";
	return {
		valid: false,
		hint: `Model '${modelAlias}' not found. Valid models: ${list}${truncated}. Full list: subagents_list`,
	};
}

// Cached model registry — read once at module load, refreshed on /reload
const modelRegistryCache: Set<string> | null = loadModelRegistry();
...
function startStatusRefresh(pi: ExtensionAPI) {
	...
	statusInterval = setInterval(() => {
		...
		for (const running of runningSubagents.values()) {
			observeRunningSubagent(running, now);
			const previousKind = running.statusState.currentKind;
			const { nextState, snapshot, transition } = advanceStatusState(
				running.statusState,
				now,
			);
			if (nextState.currentKind !== previousKind) {
				shouldRefreshWidget = true;
			}
			running.statusState = nextState;

			if (transition) {
				logSubagent("status_transition", {
					id: running.id,
					name: running.name,
					agent: running.agent,
...
async function launchSubagent(
	...
	logSubagent("spawn_requested", {
		id,
		name: params.name,
		agent: params.agent,
		cwd: params.cwd ?? ctx.cwd,
		fork: params.fork ?? false,
		interactive: params.interactive,
		modelOverride: params.model,
		task: previewText(params.task),
	});

	const agentDefs = params.agent ? loadAgentDefaults(params.agent) : null;
	let effectiveModel = resolveModelPreference(
		params.model,
		agentDefs?.model,
		params.agent,
	);

	// Validate the resolved model against the registry (if available)
	if (effectiveModel) {
		// Guard against empty model strings
		if (effectiveModel.trim() === "") {
			effectiveModel = undefined;
		} else {
			const validation = validateModel(effectiveModel, modelRegistryCache);
			if (!validation.valid) {
				if (params.model) {
					// Explicit override was invalid — HARD ERROR
					throw new Error(
						`Model '${params.model}' not found. ${validation.hint}`,
					);
				} else {
					// Config-derived model was invalid — WARN + FALLBACK
					console.warn(
						`[subagents] Config model '${effectiveModel}' not found, falling back`,
					);
					effectiveModel = undefined; // Let pi core use its default
				}
			}
		}
	}
	...
	logSubagent("spawn_resolved", {
		...
		agentAutoExit: agentDefs?.autoExit,
		agentSessionMode: agentDefs?.sessionMode,
		agentCli: agentDefs?.cli,
	});
	...
	logSubagent("surface_created", {
		id,
		name: params.name,
		surface,
		preCreated: surfacePreCreated,
		muxBackend: getMuxBackend(),
	});
	if (!surfacePreCreated) {
		const delayMs = getShellReadyDelayMs();
		logSubagent("shell_ready_delay", { id, name: params.name, delayMs });
	...
	if (launchBehavior.seededSessionMode) {
		logSubagent("session_seed_start", {
			id,
			name: params.name,
	...
	const modeHint = agentDefs?.autoExit
		...
		: "Complete your task. When finished, call subagent_done with a concise non-empty summary. The user can interact with you at any time.";
	const summaryInstruction = agentDefs?.autoExit
		? "Your FINAL assistant message should summarize what you accomplished. If you use subagent_done, pass the same concise final summary in its summary field."
		: "Your FINAL assistant message (before calling subagent_done or before the user exits) should summarize what you accomplished. If you call subagent_done, its summary field must be non-empty.";
		...
		pi.registerTool({
			...
			async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
				...
				try {
					...
					// Catch model validation errors and return structured error
					...
					if (errMsg.includes("Model '") && errMsg.includes("not found")) {
						return {
							content: [
								{
									type: "text",
									text: `Model error: ${errMsg}`,
								},
							],
							details: { error: "model_validation", model: params.model },
						};
					}
				...
				watchSubagent(running, watcherAbort.signal)
					.then((result) => {
						...
						if (result.ping) {
							...
							const sessionRef = `\n\nSession: ${result.sessionFile}\nResume: pi --session ${result.sessionFile}`;
							logSubagent("steer_delivery_start", {
								id: running.id,
								name: running.name,
								type: "subagent_ping",
								sessionFile: result.sessionFile,
					...
					.catch((err) => {
						updateWidget();
						logSubagent("watcher_promise_error", {
							id: running.id,
							name: running.name,
							agent: running.agent,
							error: err?.message ?? String(err),
