...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description:
      ...
      "You must provide a concise non-empty summary of what you accomplished.",
    parameters: Type.Object({
      summary: Type.String({
        minLength: 1,
        description: "Concise non-empty summary to return to the parent session.",
      }),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      const sessionFile = process.env.PI_SUBAGENT_SESSION;
      ...
      if (!summary) {
        return {
          content: [
            {
              type: "text",
              text: "Cannot finish without a summary. Call subagent_done again with a concise non-empty summary.",
            },
          ],
          details: { error: "summary_required" },
        };
      }

      recorder.subagentDone();
      if (sessionFile) {
        writeFileSync(`${sessionFile}.exit`, JSON.stringify({ type: "done", summary }));
      }
      ctx.shutdown();
      return {
