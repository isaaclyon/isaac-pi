...
```bash
gh pr view --json mergeable,mergeStateStatus -q '.mergeStateStatus'
```

| Status | Action |
|--------|--------|
| `DIRTY` or `BLOCKED` | **STOP** - Report conflicts, provide: `git fetch && git merge origin/main` |
| `BEHIND` | Warn user, suggest merge, continue |
...
| Merge conflicts | mergeStateStatus = DIRTY | Stop, provide rebase command |
| Branch behind | mergeStateStatus = BEHIND | Warn, continue |
| CI fails | gh pr checks exit 1 | Stop, list failed checks |
| PR already exists | gh pr view succeeds | Use existing PR |
...
| Check conflicts | `gh pr view --json mergeStateStatus -q '.mergeStateStatus'` |
