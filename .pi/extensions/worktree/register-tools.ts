...
function parseWorktreeCommandArgs(args: string): { params?: WorktreeToolParams; error?: string } {
	...
	if (action === "sync") {
		...
		if (rest[0] === "--branch") {
			if (!rest[1]) return { error: `Missing value for --branch.\n\n${usageText()}` };
			if (rest.length > 2) return { error: `Unexpected arguments for sync.\n\n${usageText()}` };
			return { params: { action, branch: rest[1] } };
		}
	...
	for (let i = 0; i < rest.length; i += 1) {
		...
		if (token === "--base") {
			...
			if (!value) return { error: `Missing value for --base.\n\n${usageText()}` };
			base = value;
			i += 1;
			continue;
		}
		...
		if (token === "--config" || token === "--config-file") {
			const value = rest[i + 1];
			if (!value) return { error: `Missing value for ${token}.\n\n${usageText()}` };
