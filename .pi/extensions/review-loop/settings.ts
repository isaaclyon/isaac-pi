...
function findProjectPiDir(startDir: string): string {
  ...
  while (true) {
    ...
    if (parent === current) {
      return join(resolve(startDir), ".pi");
    }
    current = parent;
  }
}

const PROJECT_PI_DIR = findProjectPiDir(process.cwd());
const PROJECT_ROOT = dirname(PROJECT_PI_DIR);

export const SETTINGS_PATH = join(PROJECT_PI_DIR, "settings.json");
...
function loadPatterns(config: PatternConfig | undefined, defaults: RegExp[]): RegExp[] {
  ...
  const userPatterns = config.patterns.map(parsePattern).filter((p): p is RegExp => p !== null);
...
export function getReviewPrompt(config: ReviewPromptConfig): string {
  switch (config.type) {
    ...
    case "template": {
      const templatePath = join(PROJECT_PI_DIR, "prompts", `${config.value}.md`);
      try {
...
export function loadSettings(): ReviewerLoopSettings {
  ...
  try {
    ...
    // use defaults when project settings are absent
  }
