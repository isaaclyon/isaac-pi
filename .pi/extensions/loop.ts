/**
 * Loop Extension (with handoff-style compaction)
 *
 * Based on mitsuhiko's loop extension:
 * https://github.com/mitsuhiko/agent-stuff/blob/main/pi-extensions/loop.ts
 *
 * Modifications:
...
function getHandoffCompactionInstructions(mode: LoopMode, condition?: string): string {
	const conditionText = getConditionText(mode, condition);
	return [
		`This is an automated loop working toward: ${conditionText}.`,
