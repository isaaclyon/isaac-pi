...
type ToolUpdateCallback = (partialResult: ToolUpdatePayload) => void;

interface RuntimeToolContext {
  cwd: string;
}

interface NormalizedToolExecuteArgs {
...
function isToolContextLike(value: unknown): value is RuntimeToolContext {
  if (typeof value !== 'object' || value === null) return false;
...
function normalizeToolExecuteArgs(arg3: unknown, arg4: unknown, arg5: unknown): NormalizedToolExecuteArgs {
...
function buildLimitNotice(total: number, limit: number, toolName: string): string {
...
export default function (pi: ExtensionAPI) {
  ...
  const ensureCwd = (ctx: { cwd: string }) => {
    if (!cwd) {
      cwd = ctx.cwd;
  ...
  const ensureIndex = async (
    ...
    if (await query.indexExists()) return;
  ...
  const runIndexGeneration = async (
    caller: string,
    signal?: AbortSignal,
    onProgress?: (message: string) => void,
  ) => {
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, onUpdate, ctx, signal) {
      ...
      const { onUpdate: onUpdateFn, ctx: toolCtx, signal: signalArg } = normalizeToolExecuteArgs(
        onUpdate,
        ctx,
        signal,
      );
      ...
      const runQuery = async () => {
        ...
        const lines = results.map((r) => {
          const locationStr =
            r.line !== undefined && r.character !== undefined
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      limit: Type.Optional(
        Type.Number({ description: 'Maximum results', default: 50, minimum: 1, maximum: 500 }),
      ),
    }),
    async execute(toolCallId, params, onUpdate, ctx, signal) {
      ...
      const runQuery = async () => {
        ...
        return {
          content: [{ type: 'text' as const, text: finalOutput }],
          details:
            truncation.truncated || limitedByParam
    ...
    async execute(toolCallId, params, onUpdate, ctx, signal) {
      ...
      const runQuery = async () => {
        ...
        if (symbols.length === 0) {
          ...
          return {
            content: [{ type: 'text' as const, text: `No symbols found in '${params.file}'` }],
            details: [],
          };
        }
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      limit: Type.Optional(
        Type.Number({ description: 'Maximum results', default: 20, minimum: 1, maximum: 100 }),
      ),
    }),
    async execute(toolCallId, params, onUpdate, ctx, signal) {
      ...
      const runQuery = async () => {
        ...
        if (limitedResults.length === 0) {
          logger.log({ source: 'tool', action: 'query_complete', tool: toolName, hits: 0 });
          return {
            content: [{ type: 'text' as const, text: `No symbols matching '${params.query}'` }],
            details: [],
          };
        }
  ...
  function renderTree(tree: CodeTreeNode[], depth: number): string {
    const lines: string[] = [];

    const printNode = (node: CodeTreeNode, indent: number, prefix: string) => {
