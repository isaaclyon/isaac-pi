...
import { SUBAGENT_TYPES, type BuiltinSubagentType, type SubagentType, type ThinkingLevel, type CustomAgentConfig, type JoinMode, type AgentRecord } from "./types.js";
import { GroupJoinManager } from "./group-join.js";
import { getAvailableTypes, getCustomAgentNames, getCustomAgentConfig, getVisibleBuiltinTypes, isHiddenBuiltinType, isValidType, registerCustomAgents, setHiddenBuiltinTypes, BUILTIN_TOOL_NAMES } from "./agent-types.js";
...
const HIDDEN_BUILTINS_SETTINGS_KEY = "piSubagentsHiddenBuiltinAgents";

function loadHiddenBuiltinAgentTypes(): BuiltinSubagentType[] {
  try {
    const settingsPath = join(homedir(), ".pi", "agent", "settings.json");
    if (!existsSync(settingsPath)) return [];
    const raw = readFileSync(settingsPath, "utf-8");
    const parsed = JSON.parse(raw) as Record<string, unknown>;
    const value = parsed[HIDDEN_BUILTINS_SETTINGS_KEY];
    if (!Array.isArray(value)) return [];
    const hidden = new Set<string>(value.filter((item): item is string => typeof item === "string"));
    return SUBAGENT_TYPES.filter((type) => hidden.has(type));
  } catch {
    return [];
  }
}
...
export default function (pi: ExtensionAPI) {
  ...
  const reloadCustomAgents = () => {
    setHiddenBuiltinTypes(loadHiddenBuiltinAgentTypes());
  ...
  const BUILTIN_TYPE_DESCRIPTIONS: Record<BuiltinSubagentType, string> = {
  ...
  const buildTypeListText = () => {
    const visibleBuiltins = getVisibleBuiltinTypes();
    const builtinDescs = visibleBuiltins.map((type) => `- ${type}: ${BUILTIN_TYPE_DESCRIPTIONS[type]}`);
    ...
    return [
      "Built-in types:",
      ...(builtinDescs.length > 0 ? builtinDescs : ["- (none; hidden via settings)"]),
...
- Use join_mode to control how background completion notifications are delivered. By default (smart), 2+ background agents spawned in the same turn are grouped into a single notification. Use "async" for individual notifications or "group" to force grouping.`,
    parameters: Type.Object({
      ...
      subagent_type: Type.String({
        description: `The type of specialized agent to use. Built-in: ${getVisibleBuiltinTypes().join(", ") || "(none; hidden via settings)"}. Custom agents from .pi/agents/*.md (project) or ~/.pi/agent/agents/*.md (global) are also available.`,
      }),
      model: Type.Optional(
        Type.String({
          description:
    ...
    execute: async (_toolCallId, params, signal, onUpdate, ctx) => {
      ...
      if (isHiddenBuiltinType(subagentType)) {
        return textResult(`Agent type "${params.subagent_type}" is disabled via ~/.pi/agent/settings.json key "${HIDDEN_BUILTINS_SETTINGS_KEY}".`);
      }
  ...
  async function showAgentsMenu(ctx: ExtensionCommandContext) {
    ...
    const visibleBuiltins = getVisibleBuiltinTypes();
    const maxBuiltin = visibleBuiltins.length > 0 ? Math.max(...visibleBuiltins.map(t => t.length)) : 0;
    const builtinLines = visibleBuiltins.map(t => {
    ...
    const builtinSection = builtinLines.length > 0
      ? `Built-in:\n${builtinLines.join("\n")}`
      : "Built-in:\n  (none; hidden via settings)";

    ctx.ui.notify(
      `${noAgentsMsg}${builtinSection}`,
      "info",
    );
